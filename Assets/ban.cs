...
public class ban : MonoBehaviour
{
    ...
    IEnumerator DanBan(GameObject obj, Vector3 direction)
    {
        ...
        while (obj != null && Time.time < startTime + maxTime && obj.transform.position.x < Camera.main.ViewportToWorldPoint(new Vector2(1f, 0f)).x && obj.transform.position.x > Camera.main.ViewportToWorldPoint(new Vector2(0f, 0f)).x)
        {
